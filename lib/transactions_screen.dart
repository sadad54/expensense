...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  Future<void> _handleScanReceipt(String rawText) async {
    ...
    final DateTime timestamp = DateTime.tryParse(data['date']) ?? DateTime.now();
  ...
  Future<void> _showManualEntryDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Log Transaction Manually', style: TextStyle(color: Colors.white)),
        content: Form(
          key: _formKey,
          child: Column(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              if (_formKey.currentState?.validate() ?? false) {
                ...
                final uid = _auth.currentUser?.uid;
                if (uid == null || category == null || amount == null) return;
        ...
        children: [
          ...
          ListTile(
            ...
            onTap: () async {
              final uid = _auth.currentUser?.uid;
              if (uid != null) {
                await _db.collection('users').doc(uid).collection('transactions').doc(txn.id).delete();
                setState(() {
  ...
  Widget build(BuildContext context) {
    final filteredTxns = _transactions.where((txn) {
      final matchesSearch = txn.rawText.toLowerCase().contains(_searchQuery.toLowerCase());
      final matchesCategory = _selectedCategory == 'All' || txn.categoryId == _selectedCategory;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.add, color: Colors.white), tooltip: 'Manual Entry', onPressed: _showManualEntryDialog),
          IconButton(icon: const Icon(Icons.camera_alt_outlined, color: Colors.white), tooltip: 'Scan Receipt', onPressed: () async {
                        ...
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(DateFormat.yMMMd().format(_selectedDate), style: const TextStyle(color: Colors.white)),
                            ],
                          ),
                        ),
                      ),
                      ...
                      child: Row(
                        children: [
                          Container(
                            ...
                            child: const Icon(Icons.receipt_long, color: Colors.white70),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(txn.categoryId, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 16)),
                                const SizedBox(height: 5),
                                Text(DateFormat.jm().format(txn.timestamp), style: TextStyle(color: Colors.grey[400], fontSize: 13)),
                              ],
                            ),
                          ),
                          const SizedBox(width: 8),
                          Text("-\$${txn.amount.toStringAsFixed(2)}", style: const TextStyle(color: Colors.redAccent, fontSize: 16, fontWeight: FontWeight.w600)),
                        ],
                      ),
                    ),
                  ),
